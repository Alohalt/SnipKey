...
当前分支包含原生 Windows 应用，位于 `Windows/SnipKey.Windows`。它使用 .NET 8 WPF 实现托盘应用，并保持与 macOS 版一致的 `snippets.json` 数据结构。
...
├── Windows/
│   └── SnipKey.Windows/  # 原生 Windows 应用：托盘、键盘 hook、设置页、剪贴板记录、补全面板
├── Tests/
...
make windows-build # 在安装 .NET 8 SDK 的 Windows 机器上构建 Windows 应用
make windows-run  # 在 Windows 上运行 Windows 应用
make windows-publish # 发布 win-x64 构建产物到 .build/windows-publish
...
Windows 对应文件位于 `%APPDATA%\SnipKey\`：`snippets.json`、`clipboard-history.json` 和 `app-settings.json`。
