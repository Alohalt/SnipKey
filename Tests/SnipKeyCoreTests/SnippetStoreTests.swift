...
final class SnippetStoreTests: XCTestCase {
    ...
    func testAddSnippet() {
        ...
        XCTAssertTrue(store.addSnippet(snippet))
    ...
    func testUpdateSnippet() {
        ...
        XCTAssertTrue(store.updateSnippet(snippet))
        XCTAssertEqual(store.snippets.first?.replacement, "new@example.com")
    }

    func testAddSnippetRejectsDuplicateTriggerCaseInsensitively() {
        XCTAssertTrue(store.addSnippet(Snippet(trigger: "email", replacement: "a")))

        let didAddDuplicate = store.addSnippet(Snippet(trigger: "EMAIL", replacement: "b"))

        XCTAssertFalse(didAddDuplicate)
        XCTAssertEqual(store.snippets.count, 1)
    }

    func testUpdateSnippetRejectsInvalidTrigger() {
        let snippet = Snippet(trigger: "email", replacement: "old@example.com")
        XCTAssertTrue(store.addSnippet(snippet))

        var invalidUpdate = snippet
        invalidUpdate.trigger = "邮箱"

        let didUpdate = store.updateSnippet(invalidUpdate)
    ...
    func testSnippetsForGroup() {
        let group = SnippetGroup(name: "Work")
        store.addGroup(group)
        XCTAssertTrue(store.addSnippet(Snippet(trigger: "a", replacement: "1", groupId: group.id)))
        XCTAssertTrue(store.addSnippet(Snippet(trigger: "b", replacement: "2", groupId: nil)))
    ...
    func testUngroupedSnippets() {
        ...
        store.addGroup(group)
        XCTAssertTrue(store.addSnippet(Snippet(trigger: "a", replacement: "1", groupId: group.id)))
        XCTAssertTrue(store.addSnippet(Snippet(trigger: "b", replacement: "2")))
    ...
    func testExportImport() throws {
        XCTAssertTrue(store.addSnippet(Snippet(trigger: "x", replacement: "y")))
    ...
    func testLoadNormalizesInvalidAndDuplicateTriggers() throws {
        ...
        try XCTUnwrap(json.data(using: .utf8)).write(to: tempURL)

        let reloadedStore = SnippetStore(fileURL: tempURL)

        XCTAssertEqual(reloadedStore.snippets.map(\.trigger), ["key", "Email", "email_2"])
    }
}
